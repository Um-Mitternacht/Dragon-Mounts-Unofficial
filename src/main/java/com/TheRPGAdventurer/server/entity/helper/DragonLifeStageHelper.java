...
import static com.TheRPGAdventurer.server.entity.helper.EnumDragonLifeStage.*;
...
import com.TheRPGAdventurer.server.util.ClientServerSynchronisedTickCount;
...
public class DragonLifeStageHelper extends DragonHelper {
    ...
    /**
     * Gets the current life stage of the dragon.
     *
    ...
    private void onNewLifeStage(EnumDragonLifeStage lifeStage, EnumDragonLifeStage prevLifeStage) {
        ...
        if (dragon.isClient()) {
            ...
            if (prevLifeStage != null && prevLifeStage == EGG && lifeStage == HATCHLING) {
                playEggCrackEffect();
                dragon.playSound(SoundEvents.BLOCK_WOOD_BREAK, 1, 1);
            }
        } else {
            // update AI
